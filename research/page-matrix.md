...
本轮基线：`research/站点完整度.json` 为 `ready` + `visitor-complete`。本轮实际验收了三条纵向旅程：Clay 搜索 → canonical 攻略 → 来源 → planner、planner 空/部分/完整结果 → 攻略 → 返回保留状态、Research 前置条件 → checklist → planner/更新。随后补入 DS_B.0.4.19 资料快照和跨会话历史/比较能力，严格门已通过。
...
| `/guides/how-to-research` | Research 如何帮助下一次 expedition？ | 系统攻略页 | Use the safe research checklist | S001/S002/S008；当前构建表格与未来版本边界分开 | implemented |
...
| `/database` | 当前构建的 Research、实体和掉落行是什么？ | 版本锁定资料库 | Browse the data reference | S001/S002/S008；二级解析快照，未来版本需重验 | implemented |
