...
本轮基线：`research/站点完整度.json` 为 `ready` + `visitor-complete`。本轮实际验收了三条纵向旅程：Clay 搜索 → canonical 攻略 → 来源 → planner、planner 空/部分/完整结果 → 攻略 → 返回保留状态、Research 前置条件 → checklist → planner/更新。随后补入 DS_B.0.4.19 资料快照和跨会话历史/比较能力；本次更新将官方编号边界推进到 v0.4.23，但不把旧快照伪装成新表，严格门仍需用本轮验收收据复核。
...
| `/guides/how-to-research` | Research 如何帮助下一次 expedition？ | 系统攻略页 | Use the safe research checklist | S001/S002/S008/S011；v0.4.23 官方修正与 DS_B.0.4.19 表格分开 | implemented |
| `/library` | 已发布的攻略里，哪一页对应当前问题？ | 攻略查找中心 | Search the guide library | S001/S002/S003；只索引已有真实页面 | implemented |
| `/updates` | 哪些结论需要在 Early Access 更新后重查？ | 日期更新入口 | Recheck a first run | S001/S002/S003/S011/S012/S013；不伪造完整开发者 changelog | implemented |
| `/database` | 版本锁定的 Research、实体和掉落行是什么？ | 版本锁定资料库 | Browse the data reference | S001/S002/S008/S011；二级解析快照，未来版本需重验 | implemented |
